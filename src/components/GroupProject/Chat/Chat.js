    ...
    let list = this.state.chatList.map((item, index) => {
      ...
      function emailcheck() {
        if (pre_email !== now_email) {
          pre_email = now_email;
          return <p style={{ margin: "5px" }}>{item.comment.username}</p>;
        }
      }
      function mycheck() {
        if (item.comment.email === my_email) {
          return (
            <div onClick={leftMouseListener}>
              ...
              <div
                className={
                  item.comment.email === my_email ? "balloonY" : "balloonN"
                }
                onContextMenu={onContextMenu}
              >
                <div className="msg_text">{item.comment.msg}</div>
              </div>
            </div>
          );
        } else {
          return (
            <div onClick={leftMouseListener}>
              <div
                ...
                onContextMenu={onContextMenu}
              >
                <div id="msg_text">{item.comment.msg}</div>
              </div>
              {addZero(date.getHours())}:{addZero(date.getMinutes())}
            </div>
          );
      ...
      return (
        <div key={index}>
          <div className="chattingView-msgline">
            <div
              style={
                item.comment.email === this.state.email
                  ? { textAlign: "right" }
                  : { textAlign: "left" }
              }
            >
